...
class VUEDAPermissionsMixin(PermissionsMixin):
    ...
    def has_perm(self, perm, obj: models.Model | None = None):
        ...
        # Layer 1: group permissions (model-level baseline)
        decision = super_value

        # Layer 2: state permissions (workflow)
        grant_or_deny = None
        has_workflow = False
        ...
        if obj:
            ...
            if row_level_permissions:
                ...
                # Layer 4: state+row (always runs when under workflow, can override state deny)
                if has_workflow:
                    result = row_level_permissions.check_instance_state(
