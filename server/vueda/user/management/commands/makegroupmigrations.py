...
class Command(BaseCommand):
    ...
    def _create_and_get_empty_migration(self, app_label):
        ...
        results = call_management_command(
            ...
            app_label,
            "--empty",
            f"--name=group_permission_migrations_{date_string}",
            "--noinput",
        )
    ...
    def _rewrite_migration(self, migration_file, changes, dependencies):
        with open(migration_file, "r+", encoding="utf-8") as f:
            ...
            lines[slots["reverse_index"]] = lines[slots["reverse_index"]].replace(
                "int", "backwards_migrate_groups_through_imports"
            )
            lines[slots["forwards_index"]] = lines[slots["forwards_index"]].replace(
                "str", "forwards_migrate_groups_through_imports"
            )
            ...
            lines[slots["p_forwards_index"]] = lines[slots["p_forwards_index"]].replace(
                "dict", "make_sure_permissions_exist"
            )

            # Add Dependencies
            lines[slots["dependencies_index"] + 1 : slots["dependencies_index"] + 1] = [
            ...
            lines[slots["import_start"] : slots["import_end"] + 1] = get_group_migration_imports(self.import_instead)
    ...
    def _get_project_dependency_migration_names_from_show_migrations(self, exclude):
        ...
        for app in django_apps.get_app_configs():
            ...
            if "site-packages" not in app_path.parts:
                app_label = app.label
                app_migrations = get_migration_names_from_show_migrations(self, app_label)
                if app_migrations:
    ...
    def _get_vueda_generated_migration_data_for_auth_user_model(self):
        ...
        migration_names = get_migration_names_from_show_migrations(self, app_label)
    ...
    def handle(self, **options):
        ...
        if not self.dry_run:
            auth_migration_names = get_migration_names_from_show_migrations(self, "auth")
