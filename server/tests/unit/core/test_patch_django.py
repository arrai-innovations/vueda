...
def test_get_permission_codename_reads_permission_names_mapping_at_call_time(settings):
    ...
    original = settings.PERMISSION_NAMES_MAPPING
    settings.PERMISSION_NAMES_MAPPING = {"add": "mutated"}
    assert auth.get_permission_codename("add", Distributor._meta) == "mutated_distributor"

    # Assigning the original back fires setting_changed a second time, which is what clears
    # patch_django's mapping cache again. The pytest `settings` fixture only restores at teardown,
    # so the revert has to be explicit for this last assertion to mean anything.
    settings.PERMISSION_NAMES_MAPPING = original
    assert auth.get_permission_codename("add", Distributor._meta) == "create_distributor"


def test_get_builtin_permissions_reads_permission_names_mapping_at_call_time(settings):
    ...
    settings.PERMISSION_NAMES_MAPPING = {"add": "mutated", "view": "read", "change": "update"}
