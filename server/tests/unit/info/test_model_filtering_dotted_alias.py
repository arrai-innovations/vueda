...
class TestModelFilteringAutoDerivedAlias:
    ...
    contain `__`; and `distributor__id`/`distributor__id__in`, a `NumericRangeFilter` and a
    `NumberArrayFilter` declared the same hand-written way. `PublicFilterAliasMixin` derives a
    dotted name from any `__`-joined declared name the same way, regardless of the filter's own
    class.
    """
    ...
    def test_model_filtering_reports_the_derived_dotted_names(self, api_client, settings):
        ...
        `model_filtering` reports it under -- survives being renamed to `tangible_type.code`, and a
        range filter (`distributor__id`) and an array filter (`distributor__id__in`) are renamed the
        same way a `CharFilter` is, keeping their own `type_filter`/`suffixes` metadata intact."""
