...
class TestWarningConfirmationGate:
    def test_create_without_warnings_saves(self, superuser_client):
        response = superuser_client.post(LIST_URL, data={"name": "fine", "count": 3}, format="json")
    ...
    def test_create_with_warning_and_no_acknowledgement_returns_409_without_saving(self, superuser_client):
        response = superuser_client.post(LIST_URL, data={"name": "risky", "count": -1}, format="json")
    ...
    def test_create_with_correct_acknowledgement_saves(self, superuser_client):
        payload = {"name": "risky", "count": -1}
        first = superuser_client.post(LIST_URL, data=payload, format="json")
        assert first.status_code == HTTPStatus.CONFLICT, response_body(first)
        digest = first.data["digest"]

        confirmed = superuser_client.post(LIST_URL, data=payload, format="json", HTTP_ACKNOWLEDGE_WARNINGS=digest)

        assert confirmed.status_code == HTTPStatus.CREATED, response_body(confirmed)
        assert Thing.objects.filter(name="risky", count=-1).exists()

    def test_create_with_stale_acknowledgement_re_prompts(self, superuser_client):
    ...
    def test_blocking_errors_take_precedence_over_warnings(self, superuser_client):
        # name == "blocked" raises in validate(); the warning gate is never reached.
        response = superuser_client.post(LIST_URL, data={"name": "blocked", "count": -1}, format="json")
...
class TestConfirmRequiredAction:
    def test_unacknowledged_submit_returns_409_with_custom_message(self, superuser_client):
        original_count = 5
        thing = Thing.objects.create(name="thing", count=original_count)

        response = superuser_client.post(thing_action_url("reset-count", thing.pk), format="json")
    ...
    def test_acknowledged_submit_runs_the_body(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=5)
        url = thing_action_url("reset-count", thing.pk)
        gated = superuser_client.post(url, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)

        confirmed = superuser_client.post(url, format="json", HTTP_ACKNOWLEDGE_WARNINGS=gated.data["digest"])

        assert confirmed.status_code == HTTPStatus.OK, response_body(confirmed)
        thing.refresh_from_db()
        assert thing.count == 0

    def test_stale_acknowledgement_re_prompts(self, superuser_client):
        original_count = 5
        thing = Thing.objects.create(name="thing", count=original_count)
    ...
    def test_default_confirm_message_when_none_authored(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=5)
    ...
    def test_gate_raises_even_during_dry_run(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=5)

        response = superuser_client.post(thing_action_url("reset-count", thing.pk), format="json", HTTP_DRY_RUN="true")
...
class TestExplicitGateInActionBody:
    def test_invalid_input_returns_400_before_the_gate(self, superuser_client):
        original_count = -3
        thing = Thing.objects.create(name="thing", count=original_count)  # would warn, but validation fails first

        response = superuser_client.post(thing_action_url("adjust-count", thing.pk), data={"amount": 0}, format="json")
    ...
    def test_valid_input_without_warnings_saves(self, superuser_client):
        original_count = 3
        amount = 2
        thing = Thing.objects.create(name="thing", count=original_count)

        response = superuser_client.post(
            thing_action_url("adjust-count", thing.pk), data={"amount": amount}, format="json"
        )
    ...
    def test_valid_input_with_warnings_gates_then_saves_on_acknowledgement(self, superuser_client):
        original_count = 3
        amount = -5
        thing = Thing.objects.create(name="thing", count=original_count)
        ...
        gated = superuser_client.post(url, data=payload, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
        ...
        confirmed = superuser_client.post(
            url, data=payload, format="json", HTTP_ACKNOWLEDGE_WARNINGS=gated.data["digest"]
        )
        assert confirmed.status_code == HTTPStatus.OK, response_body(confirmed)
        thing.refresh_from_db()
        assert thing.count == original_count + amount
...
class TestDestroyWarnings:
    def test_single_destroy_is_gated_then_deletes_on_acknowledgement(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=2)
        url = detail_url(thing.pk)

        gated = superuser_client.delete(url)
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
        ...
        confirmed = superuser_client.delete(url, HTTP_ACKNOWLEDGE_WARNINGS=gated.data["digest"])
        assert confirmed.status_code == HTTPStatus.NO_CONTENT, response_body(confirmed)
        assert not Thing.objects.filter(pk=thing.pk).exists()

    def test_single_destroy_without_warnings_deletes(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=0)

        response = superuser_client.delete(detail_url(thing.pk))

        assert response.status_code == HTTPStatus.NO_CONTENT, response_body(response)
        assert not Thing.objects.filter(pk=thing.pk).exists()

    def test_bulk_destroy_is_gated_then_deletes_on_acknowledgement(self, superuser_client):
        ...
        gated = superuser_client.delete(LIST_URL, data=payload, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
        ...
        confirmed = superuser_client.delete(
            LIST_URL, data=payload, format="json", HTTP_ACKNOWLEDGE_WARNINGS=gated.data["digest"]
        )
        assert confirmed.status_code == HTTPStatus.NO_CONTENT, response_body(confirmed)
        assert Thing.objects.count() == 0

    def test_bulk_destroy_without_warnings_deletes(self, superuser_client):
        alpha = Thing.objects.create(name="alpha", count=0)
        beta = Thing.objects.create(name="beta", count=0)

        response = superuser_client.delete(LIST_URL, data={"pks": [alpha.pk, beta.pk]}, format="json")

        assert response.status_code == HTTPStatus.NO_CONTENT, response_body(response)
        assert Thing.objects.count() == 0

    def test_single_destroy_validation_400_takes_precedence_over_warnings(self, superuser_client):
    ...
    def test_bulk_destroy_missing_pk_400_takes_precedence_over_warnings(self, superuser_client):
        thing = Thing.objects.create(name="thing", count=2)  # would warn, but the missing pk fails first
        missing_pk = thing.pk + 1000

        response = superuser_client.delete(LIST_URL, data={"pks": [thing.pk, missing_pk]}, format="json")
...
class TestDeactivateActivateWarnings:
    def test_bulk_deactivate_is_gated_then_proceeds_on_acknowledgement(self, superuser_client):
        ...
        gated = superuser_client.patch(BULK_DEACTIVATE_URL, data=payload, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
        assert gated.data["warnings"] == {
        ...
        confirmed = superuser_client.patch(
    ...
    def test_single_deactivate_is_gated_then_proceeds_on_acknowledgement(self, superuser_client):
        gadget = Gadget.objects.create(name="critical-solo")
        url = gadget_action_url("deactivate", gadget.pk)

        gated = superuser_client.patch(url, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
        ...
        confirmed = superuser_client.patch(url, format="json", HTTP_ACKNOWLEDGE_WARNINGS=gated.data["digest"])
        assert confirmed.status_code == HTTPStatus.OK, response_body(confirmed)
        gadget.refresh_from_db()
        assert gadget.is_active is False

    def test_deactivate_without_warnings_proceeds(self, superuser_client):
        gadget = Gadget.objects.create(name="plain")

        response = superuser_client.patch(gadget_action_url("deactivate", gadget.pk), format="json")

        assert response.status_code == HTTPStatus.OK, response_body(response)
        gadget.refresh_from_db()
        assert gadget.is_active is False

    def test_bulk_deactivate_validation_400_takes_precedence_over_warnings(self, superuser_client):
        active = Gadget.objects.create(name="critical-active")
        inactive = Gadget.objects.create(name="critical-inactive", is_active=False)

        response = superuser_client.patch(BULK_DEACTIVATE_URL, data={"pks": [active.pk, inactive.pk]}, format="json")
    ...
    def test_bulk_deactivate_of_one_object_still_uses_per_object_warnings_shape(self, superuser_client):
        ...
        gated = superuser_client.patch(BULK_DEACTIVATE_URL, data={"pks": [gadget.pk]}, format="json")
    ...
    def test_bulk_activate_is_gated_then_proceeds_on_acknowledgement(self, superuser_client):
        ...
        gated = superuser_client.patch(BULK_ACTIVATE_URL, data=payload, format="json")
        assert gated.status_code == HTTPStatus.CONFLICT, response_body(gated)
    ...
    def test_single_activate_without_warnings_proceeds(self, superuser_client):
        gadget = Gadget.objects.create(name="plain", is_active=False)

        response = superuser_client.patch(gadget_action_url("activate", gadget.pk), format="json")
