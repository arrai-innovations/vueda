...
When `Customer` reaches its formatted name through `formatted_name_lookup_expression = "data__formatted_name"`, `customer__formatted_name` names no column the database knows: the annotation `VuedaViewSet.get_queryset` adds belongs to the `Cart` queryset, not to the `Customer` rows it joins. `VuedaOrderingFilter` rewrites the path to `customer__data__formatted_name` before the query runs, and `FormattedNamePathFilterSetMixin` does the same for a filter's `field_name`. Both VUEDA filterset bases — `VuedaFilterSet` and `VuedaCompositePrimaryKeyFilterSet` — already include that mixin, so `CartFilterSet` above needs nothing extra; a filterset built on django-filter's `FilterSet` directly has to mix it in itself, ahead of the `FilterSet` base (see [The filterset half of the rewrite](../core-concepts/filtering-and-ordering-semantics#the-filterset-half-of-the-rewrite)). For ordering, a client sends the dotted form of the declared path — `?o=customer.formatted_name` — which is what `model_ordering` reports too. For filtering, `customer_name` has no `__` in it to translate, so a client sends that same declared name, which is what `model_filtering` reports and a generated filter label is built from; a filter declared against a `Meta.fields` entry that names the relation directly (`customer__formatted_name`, with no custom attribute name of its own), or one declared by hand under a `__`-joined attribute name of its own, would instead get `customer.formatted_name` derived for it automatically (see [Public filter names](../core-concepts/filtering-and-ordering-semantics#public-filter-names)). Either way, the resolved path is server-side only and is not accepted as a query parameter.
