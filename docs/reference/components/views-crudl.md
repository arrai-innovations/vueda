...
<VuedaDemo class="flex flex-col gap-3">
  ...
  <div class="rounded-vueda-card hairline hairline-border bg-card overflow-clip">
    ...
    <div class="flex flex-wrap items-center gap-2 border-b-hairline bg-muted/30 px-4 py-2">
      ...
      <Button size="sm" emphasis="ghost" class="text-xs">Reset sort</Button>
    </div>
  ...
  <footer class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
    ...
    <span>sort chips: click to toggle direction, ✕ to remove (hidden with only one chip left); with more than one sort, drag a chip by its grip handle to reorder. Reset sort shows only while the active sort differs from the model's default</span>
