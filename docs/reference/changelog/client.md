...
- **Framework-owned sticky page chrome (new `StickyStackProvider`, `StickyChrome`, `useStickyStack`)**:
    - New opt-in layout primitives for pinning page chrome. `StickyStackProvider` wraps the scrolling region (the window keeps scrolling; the provider adds no `overflow`) and renders a top zone pinned to the top of the viewport and a bottom zone pinned to the bottom. Place the page title in its `top` slot. `StickyChrome` teleports a view's chrome into a zone (`zone="top"` / `zone="bottom"`) and sets that zone's reveal behavior (a `useScrollReveal` strategy or a boolean), degrading to in-place rendering when no provider is present. `useStickyStack` is the underlying context (provider role establishes it; view role registers chrome), mirroring `usePageTitle`.
    - The provider publishes the measured top-zone height as the `--vueda-sticky-stack-top` custom property (collapsing to `0` while the top zone is hidden) for a sticky grid header to offset against, and warns in development when an ancestor's non-visible `overflow` would silently break window-relative `position: sticky`.
