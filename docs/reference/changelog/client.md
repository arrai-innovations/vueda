...
- **List and form chrome migrated onto the sticky stack (ViewList, ViewCreate, ViewUpdate, ViewRead, DetailView, StickyBar; integrator templates)**:
    - `ViewList` now teleports its under-actions toolbar into the top zone (revealing on scroll up) and its pagination footer into the bottom zone (always shown) through `StickyChrome`. With no `StickyStackProvider` present, both render inline where they sit today, so lists without the provider are unchanged.
    ...
    - The `integrator-monorepo` and `integrator-monorepo-dx` templates now wrap `<RouterView>` in `StickyStackProvider` with `PageTitle` in the `top` slot, the same wiring the widget-warehouse demo uses.
      _If you render `ViewList`, `ViewCreate`, `ViewUpdate`, `ViewRead`, or `DetailView` in a shell without a `StickyStackProvider`, their toolbar, action bar, and pagination now render inline (in normal flow) instead of pinned. To keep them pinned, place a `StickyStackProvider` around your routed content with `PageTitle` (or your own title component) in its `top` slot; see the updated templates. Standalone `<StickyBar>` usage and the `scrollRoot` prop are unchanged._
