...
- **History records the action behind a change**:
    ...
    - `vueda.core.audit.audited_action` provides the same grouping where there is no request. VDQ tasks now run inside one, named for the task. A write outside any action still records its event, with no action identity to associate it with the others.
      _Wrap a management command, a background job, or a service operation that should read as one action in `audited_action("your.action")`. It does nothing when `vueda.history` is absent, so a caller in an optional app needs no guard._
