...
- **History records the action behind a change**:
    - Installing `vueda.history` adds `vueda.history.middleware.VuedaHistoryMiddleware` to `MIDDLEWARE`. It records the acting user, the request path, and the request method, so every event one request produces shares one action identity while remaining a separate row.
