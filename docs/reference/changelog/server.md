...
- **Requestable `column_totals` subset**:
    - A `list` request can now request a subset of a viewset's declared `column_totals` by naming them in `FIELDS_PARAM` alongside the row fields it wants; `columnTotals` in the response contains only the requested keys. Naming a `column_totals` field in `FIELDS_PARAM` no longer raises an invalid-field error and no longer adds a matching column to the objects in `results`. Submitting a wildcard `FIELDS_PARAM` value returns every declared total. Omitting `FIELDS_PARAM` entirely now returns no totals (`columnTotals` is `{}`) and skips the aggregation query, instead of returning every declared total.
    - `column_totals` entries can now be double-underscore relation lookups (for example, `product_option__quantity_available`), not just fields declared directly on the viewset's model, since the aggregation runs against the queryset rather than the serializer's field list.
