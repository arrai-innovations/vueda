...
- **`model_fields` metadata resolved through the wrong attribute**:
    ...
    - Added the `vueda_info.W001` system-check warning, reported by `manage.py check`. It flags a non-`SerializerMethodField` field whose `source` or model `<field>_lookup_expression` fails to resolve to a model field, naming the serializer, field, and path. It is advisory only and never blocks `/info/` or a management command from completing. The two mechanisms are gated differently: a `source` that fails at its very first segment (an annotated value, a `@property`-backed source) is not flagged, since that pattern is common and intentional, and only a `source` that resolves partway before breaking is reported; a `<field>_lookup_expression` is flagged on any failure, including at its first segment, since it is fed directly to `models.F()` for queryset annotation and to Django admin's `lookup_field()` and so has no legitimate non-model-backed reading the way `source` does.
