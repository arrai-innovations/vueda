...
- **Object history is returned as the user actions behind it**:
    ...
    - `VuedaHistoryViewSet` is removed. `VuedaViewSet` carries the history endpoint, and it offers no history action, model-info metadata, or route for a model that records none. `VuedaHistorySerializer` moves from `vueda.core.serializers` to `vueda.history.serializers`.
    - `SimpleHistoryModelMixin` and `ProxyAwareHistoricalRecords` move from `vueda.history.models` to `vueda.core.simple_history`. `vueda.workflow` tracks its own models through that mixin. The generated `Historical*` models keep the `vueda_workflow` app label, and this change generates no migrations.
      _Replace `VuedaHistoryViewSet` with `VuedaViewSet`; the history endpoint comes with it. Read `object_revision` where you read `current_history_id`, and compare it against an event's own identifier instead of calling the removed `current` action. Update `from vueda.core.serializers import VuedaHistorySerializer` to `from vueda.history.serializers import VuedaHistorySerializer`. A project that imported the simple-history mixin or records class from `vueda.history.models` should import it from `vueda.core.simple_history` instead. `VuedaHistoryModel` stays in `vueda.history.models`._
