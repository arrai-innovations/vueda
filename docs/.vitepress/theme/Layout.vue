...
const hasBreadcrumb = computed(() => {
    ...
    const route = relativePath.endsWith("/index.md")
    ...
    return normalized.slice(1).split("/").length > 1;
});
</script>
...
<template>
    <div
        ...
        class="Layout"
        :class="[frontmatter.pageClass, { 'has-breadcrumb': hasBreadcrumb }]"
    >
...
@media (min-width: 960px) {
    .Layout.has-breadcrumb :deep(.VPContent) {
        padding-top: calc(var(--vp-nav-height) + var(--vueda-breadcrumb-height));
    }

    .Layout.has-breadcrumb :deep(.VPLocalNav) {
        top: calc(var(--vp-nav-height) + var(--vueda-breadcrumb-height));
    }
}

.Layout {
    --vueda-breadcrumb-height: 36px;
}
</style>
