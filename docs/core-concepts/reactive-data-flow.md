...
**Normalization.** Model-info normalization is the most involved. `storeModelInfo` strips `model_` prefixes from top-level keys, rewrites `expands` to `expand`, camelCases nested objects for `fields`, `filtering`, and nested payloads, and identifies the primary key field by scanning `data.fields` for an entry with `{pk: true}`. If no PK field is found, the normalization throws an error, which is cached (see below). Root-level keys such as `verbose_name` and `verbose_name_plural` are not camelCased; they retain their wire-format casing. This asymmetry is intentional but creates an observable casing boundary: nested field metadata uses camelCase, while root model-info keys use snake_case.
