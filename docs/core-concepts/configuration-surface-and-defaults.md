...
**Permission mapping drift.** Omitting the `vueda.core.patch_django` import leaves `get_permission_codename` and `get_builtin_permissions` unpatched: permissions are created and checked under Django's default codenames (`add_*`, `view_*`, `change_*`) rather than VUEDA's CRUDL names. The symptom is authorization failures that seem unrelated to the actual permission assignments. If `patch_django` is imported but a reverse mapping is finalized in settings only after that import, `ObjectPermissions.perms_map` keeps the rewrite selected at import time, which can leave HTTP-method-to-codename resolution out of sync with the mapping actually in effect. This drift does not reach `DynamicObjectPermissions` or `WorkflowObjectPermissions`, which resolve their CRUDL action through the mapping at check time.
