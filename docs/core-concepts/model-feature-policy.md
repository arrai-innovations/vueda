...
The client reads the same flag. Its workflow store requests transitions, states, and history only for a model whose model info reports `workflow_enabled: true`, and returns an empty list for any other model without a request. The flag controls which requests the client sends. The server still enforces workflow permissions and transition rules.
