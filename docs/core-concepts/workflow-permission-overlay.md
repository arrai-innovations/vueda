...
Standard retrieve, update, partial-update, and destroy actions defer to DRF's object permission check. A custom action may declare itself in the viewset's `workflow_object_permission_actions` collection only when the action always performs an object permission check. A detail route alone is not sufficient because DRF does not require every detail action to fetch its object through `get_object()`. `VuedaViewSet.history_list` declares itself this way by default: its body opens with `get_object()`, so a requester whose read comes only from a matching workflow-state grant reads an object's history the same way that requester reads the object itself. A subclass that declares its own further action in this collection keeps `history_list`'s entry too; `VuedaViewSet` merges its own default into whatever a subclass sets there rather than letting the subclass's value replace it.
