...
Queue Item endpoints impose an additional viewset-level gate: the `vueda_vdq.can_resend` permission must be present before any workflow endpoint (object state, permitted transitions, execute transition) processes.
