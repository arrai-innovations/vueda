...
**`model_permissions`** is a third metadata surface that is sometimes confused with the other two. It returns the content-type permission catalogue exposed for the model's canonical serializer, regardless of whether the requesting user holds those permissions. For most serializers this is the full set of permission codenames. When the canonical serializer subclasses `VuedaReadonlySerializer`, the catalogue contains only the mapped `list` and `read` permissions because the serializer exposes no write surface. This filtering shapes discovery metadata only. It does not remove other permission rows from the database or change server authorization. `model_permissions` is not user-filtered and is not an executable action list. Using it as the source of truth for action availability over-advertises capabilities: the UI may suggest operations that the user cannot perform.
...
**`model_permissions` vs `model_actions` confusion.** `model_permissions` lists the permission catalogue exposed for the canonical serializer. This is normally the full content-type catalogue, but read-only serializers expose only mapped `list` and `read` permissions. `model_actions` lists the actions the requesting user can perform. Using `model_permissions` to drive UI visibility suggests capabilities the user may not have. The correct source for action-driven UI is `model_actions` (model scope) or `available_actions` (object scope).
