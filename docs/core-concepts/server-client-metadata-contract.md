...
`type_db`/`type_model` are resolved through the serializer field's `source` (a dotted path such as `author.name` walks the named relation to its terminal field, matching a traversing `formatted_name_lookup_expression` such as `"state__name"`), not through the serializer field's own name. Both are `null` for a field whose source does not resolve to a concrete model field, and the payload carries no key distinguishing why: `/info/` stays tolerant either way, an unresolved source never raises, it just reports `null`. That `null` is an overloaded signal. A `SerializerMethodField` is never model-backed, by design, and reports `null` without comment. So does a `source` that fails to resolve at its very first segment (an annotated value such as `current_history_id`, or a `@property`-backed source such as a workflow's computed current state) — `source` is DRF's general attribute lookup, and pointing it at a computed value with no database column is routine, correct usage, not a mistake worth flagging. A `source` that resolves partway through a relation — some segment named a real relation — before a later segment breaks is a different story: that pattern is a much stronger signal of an actual typo or rename, so it surfaces through the `vueda_info.W001` system-check warning (see [Failure Modes and Recovery](#failure-modes-and-recovery) below) at `manage.py check` time, rather than requiring anyone to notice the null at request time. A `<field>_lookup_expression` failure always surfaces that way, even at its first segment: unlike `source`, it has no legitimate non-model-backed reading, since it is fed straight to `models.F()` for queryset annotation and to Django admin's `lookup_field()`, both of which resolve purely at the database level.
...
For `model_expands`, `VuedaExpandableFieldsSerializerMixin.generate_expand_model_info` walks `Meta.expandable_fields` and builds one descriptor per entry, including nested field metadata (via `get_model_fields_data`) when the entry is a real serializer class with a `Meta.model`. `SimpleHistorySerializerMixin.generate_expand_model_info` overrides this (calling `super().generate_expand_model_info()`) to fill in the `history`, `first_history_entry`, and `last_history_entry` descriptors that `VuedaHistorySerializer` adds, since none of the three is backed by a `Meta.model`-bearing nested serializer for the base generation step to derive field metadata from automatically (`history` is a `ListField` subclass, `first_history_entry`/`last_history_entry` are `SerializerMethodField`s). The fields it embeds in those three descriptors are the root model's own fields (fetched via `get_model_fields_data` against the canonical serializer), so `SimpleHistorySerializerMixin.generate_expand_model_info` also passes them through the canonical serializer instance's `get_field_model_info(fields)` before assembling each descriptor — the same correction the root `model_fields` entry receives. A serializer author who overrides `get_field_model_info` to fix a method field's metadata therefore does not need a separate fix for that field's appearance inside `history`, `first_history_entry`, or `last_history_entry`; the one override covers both. `ModelInfoSerializer.get_model_expands` calls `generate_expand_model_info()` and then passes the result through the canonical serializer instance's `get_expand_model_info(expands)`. `VuedaExpandableFieldsSerializerMixin.get_expand_model_info` is a no-op by default; serializer authors override it (without calling `super()`, since the default has nothing to add) to patch descriptors for `SerializerMethodField`-backed expandable_fields entries.
...
**Unresolved field source or lookup expression.** The `vueda_info.W001` system check reports, for each registered serializer, a non-`SerializerMethodField` field whose `source` or model `<field>_lookup_expression` fails to resolve to a model field. It names the serializer, the field, and the path that failed. Unlike `vueda_info.E001`, this is advisory: it never blocks `manage.py check`, a management command, or `/info/` from completing, since the corresponding field simply reports `null` for `type_db`/`type_model`.

The two mechanisms are gated differently, because only one of them has a legitimate reason to fail. A `source` failing at its very first segment (an annotated value, or a `@property`-backed source such as a workflow's computed current state) is not reported — that pattern is common and often intentional, since `source` is DRF's general attribute lookup and routinely points at something with no database column at all. Only a `source` that makes partial progress — some segment named a real relation — before a later segment breaks is flagged, since that is a much stronger signal of an actual typo or rename. A `<field>_lookup_expression`, by contrast, is always reported when it fails to resolve, including at its first segment: it is fed directly to `models.F()` for queryset annotation (`annotate_formatted_name` in `vueda.core.models`) and to Django admin's `lookup_field()`, both of which resolve purely at the database level and can never reach a `@property` or method, so it has no legitimate non-model-backed reading the way `source` does — any failure is a real misconfiguration that will also break whatever annotates or looks up the value it names.

Correct a flagged field's metadata with `get_field_model_info` (see [Customize Model Info Field and Expand Metadata](../guides/customize-model-info-metadata)) when it is genuinely not model-backed — an option that only ever applies to a `source=` failure, never to a `<field>_lookup_expression` failure — or fix the field's `source=`/the model's lookup expression if the break was unintentional.
