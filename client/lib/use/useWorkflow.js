...
import { inject, provide, reactive, readonly, unref } from "vue";
    ...
    if (!workflowState) {
        ...
        const fetchModelStates = async (app, model) => {
            ...
            const response = await fetch(
                `${httpOrHttpsHostname}/routes/workflows/states/${snakeCaseApp}/${snakeCaseModel}/`,
                {
                    method: "GET",
                    credentials: "include",
                },
        ...
        const executeTransition = async (stateToRoute, app, model, objectId, transition_code) => {
            const result = {
                app: unref(app),
                model: unref(model),
                id: unref(objectId),
        ...
        provide(WorkflowSymbol, workflowState);
    }
