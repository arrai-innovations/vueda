...
/**
 * A segment-based date input built on Reka UI's DateFieldRoot. Users tab
 * through individual year, month, day (and optional time) segments. Supports
 * granularity for date-only or datetime input.
 */
defineOptions({});
...
const props = defineProps({
    ...
    /** The controlled date value. Can be bound as v-model. */
...
const theme = useTheme("ControlDateField", props);
</script>

<template>
    <DateFieldRoot v-slot="slotProps" data-slot="date-field" v-bind="forwarded" :class="[theme('root'), props.class]">
