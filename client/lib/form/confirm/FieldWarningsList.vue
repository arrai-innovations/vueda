 ...
 * plain, unlabeled list; every other field renders inline (`field: message`) when it carries
...
<template>
    <div :class="[theme('root'), props.class]" data-qa="field-warnings-list">
        <template v-for="entry in entries" :key="entry.field">
            <!-- @slot [entry] Override one field's entire layout. Receives `field` and `messages` (always an array). Falls back to the header/list/inline layout below. -->
            <slot name="entry" :field="entry.field" :messages="entry.messages">
                ...
                <p v-else-if="entry.messages.length === 1" :class="theme('fieldInline')" data-qa="form-confirm-warning">
                    <span :class="theme('fieldName')">{{ entry.field }}:</span>
                    {{ entry.messages[0] }}
                </p>
                <div v-else :class="theme('field')">
                    <p :class="theme('fieldName')" data-qa="form-confirm-warning-field">{{ entry.field }}</p>
                    <ul :class="theme('list')">
                        <li v-for="(warning, index) in entry.messages" :key="index" data-qa="form-confirm-warning">
                            {{ warning }}
                        </li>
