 ...
 * to a filled CTA, and every other action rests at a quieter emphasis chosen by
 ...
 * @property {'fill' | 'outline' | 'ghost' | 'link'} [resting] - Emphasis when not promoted; defaults to `outline`.
 * @property {'neutral' | 'primary' | 'destructive'} [tone] - Explicit tone override; wins over the intrinsic tone.
 * @property {'fill' | 'outline' | 'ghost' | 'link'} [emphasis] - Explicit emphasis override; wins over promotion and resting.
 */
 ...
 * `tone` / `emphasis` override their axis entirely.
 *
...
export function resolveActionVariant({
    ...
    const resolvedEmphasis = emphasis || (primary ? "fill" : resting);
