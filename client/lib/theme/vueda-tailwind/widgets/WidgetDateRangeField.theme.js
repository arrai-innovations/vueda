/**
 * @module theme/vueda-tailwind/widgets/WidgetDateRangeField.theme
 *
 * Per-component theme registration for WidgetDateRangeField. Imported as a side effect by
 * WidgetDateRangeField.vue, so a route chunk that pulls only that SFC drags only this
...
patchTheme({
    /**
     * Date widget for date-range fields. Wraps a {@api theme-key:DateRangeField.root}
     * shell of editable segments and a calendar popover trigger.
     */
    WidgetDateRangeField: {
        /** Pass-through hook for the segment shell. The shell recipe lives on {@api theme-key:DateRangeField.root}. */
        field: {
            class: [],
        },
        /** Pass-through hook for the literal separators between date segments ("/"). Segment type comes from {@api theme-key:DateRangeFieldInput.root}. */
        literal: {
            class: [],
        },
        ...
        /** The en dash between the start and end segment groups. Muted and padded so it reads as a range connector rather than another segment. */
        separator: {
